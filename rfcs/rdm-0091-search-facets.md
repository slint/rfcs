...
[RFC before 12/06](https://codimd.web.cern.ch/RvQ_-7iDQ4K15tkJ5M2Fvw#)

## Summary

> Write this last

<!-- We distinguish (will implement?) between three different kinds of facets:
...
Hovering over the row displays the ❌ (replacing the aggregate total) which allows you to exclude the value.

![](https://codimd.web.cern.ch/uploads/upload_599eab011951a53f0843a434eb2ddb53.png)
...
---
:::spoiler 🔍 Example requests for excluding facet values
```json
...
![](https://codimd.web.cern.ch/uploads/upload_ea01fe3739173c5342bbf0134ab3825b.png)
...
![](https://codimd.web.cern.ch/uploads/upload_e0f85cbd82473c03465b42910cbc328b.png)
...
![](https://codimd.web.cern.ch/uploads/upload_310ae226f242d6a3aa373f8efc04b338.png)
...
---
:::spoiler 🔍 Example requests for fetching more values for facets at the records endpoint
```json
...
![](https://codimd.web.cern.ch/uploads/upload_02d15ec44f719377eddbbed1970707eb.png)
...
:::info
ℹ️ The REST API endpoints that serve the autocomplete results will not be affected. This will allow the UI frontend to consume the autocomplete results and display them to the user.
:::
...
---
:::spoiler 🔍 Example `SearchFacet` definitions in `SearchOptions.facets`
```python
class SearchOptions:
...
---
:::spoiler Rough implementation of `RecordService.search_facets(...) and SearchFacet.search(...)`
```python
...
---
:::spoiler 🔍 Example of executing a search query for the value "CERN" in the affiliations facet
```json
...
---
:::spoiler 🔍 Example of executing a search query for the value "parquet" in the filetype facet
```json
GET /records/_search
{
  "aggs": {
...
![](https://codimd.web.cern.ch/uploads/upload_be63730b935e1569ea7fa1f7fff91783.png)
...
---
:::spoiler Example `DateFacet` definition in `class SearchOptions`
```python
class SearchOptions:
...
- Unambiguous, especially when dealing with full dates
    - `publication_date=2014-01-01..2014-12-31` is easier to parse than `publication_date=2014-01-01-2014-12-31`
- URL-friendly, i.e. a developer can easily construct URLs in their browser, without worrying about encoding special characters
...
---
:::spoiler Partial example of this config in a community
```json
{
  ...
  "search": {
...
---
:::spoiler Example `Collection.settings` value
```json
{
  "search": {
    "facets": [
...
![](https://codimd.web.cern.ch/uploads/upload_d6b4273d12f93f9d7e7b8f1dd89d1e85.png)

### After

![](https://codimd.web.cern.ch/uploads/upload_8f2ed311c0ca2787a3d31d0528799536.png)

**After selections**

![](https://codimd.web.cern.ch/uploads/upload_7a36b3f7389266c65a91c06a58abb0c2.png)
...
:::spoiler Datamodel: Vocabulary-search facets, Direct aggregation search
...
:::spoiler Support for more filters/facets than what is displayed
...
:::spoiler  When do they exclude results
...
1. Clicking twice on the checkbox (but this is hard to discover)
2. Having confirmation on clicking the box if you want to "limit to" or "exclude" the value.

**Decision:** When users hover over the row, the ❌ appears allowing them to exclude the facet term. We color-code with yellow/warning when selected filters exclude values from the results.


:::

:::spoiler When do they narrow-down/expand results (i.e. AND/OR)
...
:::spoiler How can you search for values inside facets
...
:::spoiler How do you see the filters you have applied (and remove them)
...
:::spoiler Can you collapse facets
...
:::spoiler Can you see more results (without searching)
...
:::spoiler Ordering of terms in facets
...
:::spoiler How do you filter dates
